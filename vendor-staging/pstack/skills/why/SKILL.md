...
- `subagent_type`: `generalPurpose`
- `model`: resolve the configured `why synthesizer` role per `~/.cursor/rules/pstack-models.mdc`. Pass a real slug as `model`. Omit `model` for `inherit-parent`/`auto`. If the role line is absent, default to `claude-opus-4-8-thinking-xhigh`.
